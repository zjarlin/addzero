...
fun rememberSysRoleFormState(current:SysRoleIso?=null): MutableState<SysRoleIso> {
  ...
  onValueChange = {
     ...
     label = "角色名称" ,
        ...
        ,
            SysRoleFormProps.systemFlag to {        AddTextField(
            value = state.value.systemFlag?.toString() ?: "",
  onValueChange = {
            state.value = state.value.copy(systemFlag  =if (it.isBlank())   false  else it .parseObjectByKtx())
            },
     label = "是否为系统角色" ,
)
 }
