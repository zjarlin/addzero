...
class TableApiConfigBuilder {
    private var onLoadData: (CommonTableDaTaInputDTO) -> SpecPageResult<TableRowType> = { SpecPageResult.Companion.empty() }
...
class TableUiConfigBuilder {
    ...
    fun build() = TableUiConfig(rowHeight, headerHeight, showActions, showPagination, showBatchOperations, showAdvancedSearch)
}
...
class TableImportConfigBuilder {
    ...
    private var description: String = "请上传Excel文件(.xlsx)，文件大小不超过5MB，支持批量上传。\n请确保使用正确的模板格式，否则可能导致导入失败。"
...
class TableSearchConfigBuilder {
    ...
    fun build() = TableSearchConfig(enableKeywordSearch, enableAdvancedSearch, searchPlaceholder, autoSearch, autoSearchDelay)
}
